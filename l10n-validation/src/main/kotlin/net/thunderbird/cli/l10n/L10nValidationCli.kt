...
class L10nValidationCli : CliktCommand(name = "l10n") {
...
internal class CheckBranchCompatibilityCommand(
    ...
    private val baseRef by
        option(help = "Base Git ref used to identify changed localization source files").required()
    private val headRef by
        option(help = "Head Git ref containing the proposed changes").default("HEAD")
    private val upstreamRef by option(help = "Closest upstream release-train Git ref")
    private val downstreamRefs by
        option(
                "--downstream-ref",
                help =
                    "Downstream release-train Git ref to preserve compatibility with; may be repeated",
            )
            .multiple()
    private val allowTypoFix by
        option(help = "Allow text-only changes to keys and metadata still used downstream").flag()

    override fun help(context: Context): String =
        "Check localization source compatibility across release branches"
