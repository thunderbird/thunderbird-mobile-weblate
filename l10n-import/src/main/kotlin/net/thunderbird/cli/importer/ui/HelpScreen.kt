...
private fun HelpCommand(type: Command.Help.HelpType) {
    when (type) {
        Command.Help.HelpType.IMPORT -> {
            ...
            Text("  --config           Project config file (default: l10n-tools.json)")
            Text("  --source-repo      Source repository, format: owner/repo or Git URL")
            Text("  --branches         Comma-separated branch list")
        ...
        Command.Help.HelpType.VALIDATE -> {
            ...
            Text("  --config           Project config file (default: l10n-tools.json)")
            Text("  --source-repo      Source repository, format: owner/repo or Git URL")
            Text("  --output-dir       Output directory for validated keys (default: .)")
            Text("  --branches         Comma-separated branch list")
            Text("  --help             Show this help message")
        }
