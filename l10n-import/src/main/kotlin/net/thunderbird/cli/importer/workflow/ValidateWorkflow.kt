...
class ValidateWorkflow(private val context: ImportContext, private val gitClient: GitClient) {
    suspend fun run(observer: WorkflowObserver) {
        ...
        for (result in branchResults) {
            ...
            branchRootDirs[result.branch.value] = context.getTmpBranchRootDir(result.branch)
        ...
        val result =
            consolidator.consolidateAllFiles(
        ...
        TranslationFileWriter.writeFiles(context.projectRoot, result.files)
        ...
        observer.log("   Output directory: ${context.projectRoot.absolutePath}")
    }
}
