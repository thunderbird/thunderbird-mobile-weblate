...
val TranslationKeyConsolidatorTests by
    testSuite("TranslationKeyConsolidator") {
            ...
            try {
                ...
                val result =
                    consolidator.consolidateAllFiles(
                        ...
                        branchRootDirs =
                            mapOf("main" to mainRoot.toFile(), "beta" to betaRoot.toFile()),
                    )
                ...
                assertTrue(
                    conflict.conflicts.map { it.branch to it.content }.contains("main" to "Main")
                )
                assertTrue(
                    conflict.conflicts.map { it.branch to it.content }.contains("beta" to "Beta")
                )
            } finally {
