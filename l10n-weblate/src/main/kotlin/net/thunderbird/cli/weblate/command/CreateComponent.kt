...
class CreateComponent : BaseCommand(name = "create") {
    ...
    override fun onRun(
        ...
        val defaultComponent = allComponents.find { it.info.slug == config.toolsConfig.weblate.defaultLinkedComponent }
        if (defaultComponent == null) {
            println("    ❌ Could not find default component: ${config.toolsConfig.weblate.defaultLinkedComponent}")
            return
        }

        val weblateSlugs = allComponents.map { it.info.slug }.toSet()

        println("Found ${localComponents.size} local components with Android or Compose source strings")
        ...
        if (missingInWeblate.isNotEmpty()) {
            ...
            println("All local components with resource strings have a corresponding component in Weblate.")
        }
    }
    ...
    private fun createComponentPayload(
        ...
        val resource = config.toolsConfig.weblate.discovery.resources.first { it.type == module.type }
        val fileMask = module.path + resource.fileMaskSuffix
        val template = module.path + resource.templateSuffix
