...
object ComponentConfigDiff {
    ...
    fun computeConfigDiff(
        expected: ComponentConfig,
        actual: ComponentConfig,
        indentLevel: Int = 0,
    ): List<String> {
        return fields.mapNotNull { it.diff(expected, actual, indentLevel) }
    }

    private fun <T> value(name: String, selector: (ComponentConfig) -> T): DiffField =
        ValueField(name, selector)

    private fun set(name: String, selector: (ComponentConfig) -> List<String>): DiffField =
        SetField(name, selector)

    private fun multiline(name: String, selector: (ComponentConfig) -> String): DiffField =
        MultilineField(name, selector)

    private val fields: List<DiffField> =
        listOf(
