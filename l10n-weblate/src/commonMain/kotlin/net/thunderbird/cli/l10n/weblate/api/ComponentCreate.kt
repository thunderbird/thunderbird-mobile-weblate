 ...
 * @property fileFormat The file format (e.g., android: "aresource", compose-resource:
 *   "cmp-resource")
...
data class ComponentCreate(
    ...
    val slug: String,
    val project: String,
    @SerialName("filemask") val fileMask: String,
    val template: String,
    @SerialName("file_format") val fileFormat: String,
    val category: String? = null,
    @SerialName("linked_component") val linkedComponent: String? = null,
    val repo: String,
    val vcs: String,
    @SerialName("merge_style") val mergeStyle: String,
    val config: net.thunderbird.cli.l10n.weblate.api.ComponentConfig,
) {
    companion object ComponentCreateSerializer : KSerializer<ComponentCreate> {
        override val descriptor: SerialDescriptor =
            buildClassSerialDescriptor("ComponentCreate") {
                element<String>("name")
                element<String>("slug")
        ...
        override fun serialize(encoder: Encoder, value: ComponentCreate) {
            ...
            val json = buildJsonObject {
                ...
                val configJson =
                    encoder.json.encodeToJsonElement(ComponentConfig.serializer(), value.config)
                configJson.jsonObject.forEach { (key, jsonElement) -> put(key, jsonElement) }
            }
