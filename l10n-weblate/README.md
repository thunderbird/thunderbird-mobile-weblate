...
./scripts/weblate --token YOUR_WEBLATE_TOKEN --component-config-file ./l10n-component-config.json --log-level ALL --dry-run update
```
...
- `--component-config-file`: Path to component config JSON. Defaults to `./l10n-component-config.json`.
...
- Component config: `./l10n-component-config.json`, unless overridden with `--component-config-file`.
